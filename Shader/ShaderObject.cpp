...
void ShaderObject::setVertex(const char* name, std::vector<float>& vertices)
{
	const GLuint vertLoc = glGetAttribLocation(id, name);
	assert( isValidUniform( name, vertLoc ) );
	glVertexAttribPointer( vertLoc, 3, GL_FLOAT, GL_FALSE, 0, &(vertices.front()) );
...
void ShaderObject::setVertexAttrib(const char* name, std::vector<float>& attributes, const int size)
{
	...
	assert( isValidUniform( name, loc ) );
	glVertexAttribPointer( loc, size, GL_FLOAT, GL_FALSE, 0, &(attributes.front()) );
