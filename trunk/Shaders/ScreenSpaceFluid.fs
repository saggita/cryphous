...
void main(void)
{
	...
	vec3 backgroundColor = vec3( 0.0, 0.0, 0.0);//texelFetch( backgroundTexture, fragCoord, 0).rgb;
	...
	float specular = pow( max( 0.0, dot( normal, half)), 100.0 );
	...
	vec3 refractColor = vec3( 0.0, 0.0, 0.0);//texelFetch( backgroundTexture, fragCoord.xy + refractFrag, 0).rgb;
