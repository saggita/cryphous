...
public:
	...
	void calculatePressureForce( const ParticlePair& pair )
	{
		...
		const Geometry::Vector3d pressure = kernels.getSpikyKernelGradient( pair.getDistanceVector() ) *
	...
	void calculateViscosityForce( const ParticlePair& pair )
	{
		...
		const Geometry::Vector3d diffVector( pair.getParticleX()->velocity, pair.getParticleY()->velocity );
		const Geometry::Vector3d viscosity = viscosityCoefficient * diffVector * 
	...
	{
		const Geometry::Vector3d& kernel = kernels.getPoly6KernelGradient( pair.getDistanceVector() );
